...
namespace ProgrammingProjectTest
{
    class TemplateCreature
    {
        ...
        protected CreatureType type1;
        protected CreatureType type2;
        ...
        public TemplateCreature(int[] baseStats, string name, string ability, CreatureType type1, CreatureType type2, string[] moveIDs)
        {
    ...
    class Creature : TemplateCreature
    {
        ...
        public Creature(TemplateCreature template, MoveList moveList)
        {
            combatPrintXCoOrd = 4;
            ...
            CreateCreature(template,moveList); 

        }

        public Creature(TemplateCreature template,MoveList moveList,int forcedIV)
        {
        ...
        public void CreateCreature(TemplateCreature template,MoveList moveList)
        {
            int randomNum = 0;
            ...
            //determines which moves will always be on the Creature and then determines which of the random ones will be in the movepool
            //ids that do not begin with '!' are always in the Creature
            for (int j = 0; j < 6; j++)
            {
        ...
        public CreatureType GetType(string typeName)
        {
        ...
        public bool CanCreatureMove()
        {
            bool canCreatureMove = true;

            if (status.AfflictedCreatureCannotMove)
            {
                canCreatureMove = false;
            }
            ...
            return canCreatureMove;
        }

        public bool EndOfTurnUpdate(Creature opponent)
        {
            ...
            {
                TakeDamage(-tickDamage,ability);//negative so heals Creature
                changeHappened = true;
            }
        ...
        public int CalculateSwitchWeight(Creature opponent)
        {
        ...
        public void MatchupInspectPrint(int startX,int startY,Creature target)
        {
            InitialFormat(startX, startY);
            ...
            set
            {
                if(value.FullName != "")
                {
                    ...
                    if (char.IsDigit(value.FullName[0]))
                    {
                        ...
                        if (ability != "Clear Mind" && modifierChange > 0)
                        {
                            Console.WriteLine("Any stat drops have been prevented by " + ability);
                        }
                        else
                        {
                            string message;
                    ...
                    else
                    {
                        if (value.OverrideLevel > status.OverrideLevel)
                        {
                            ...
                            {
                                Console.WriteLine(value.FullName + " was blocked by " + name + "'s " + ability);
                            }
                        }
                    }
                }
